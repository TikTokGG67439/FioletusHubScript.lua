...
-- Injected helpers: isTargetOccludedByWall and AimGyro safe helpers
local function isTargetOccludedByWall(localHRP, targetHRP)
	...
	rp.FilterDescendantsInstances = {game:GetService('Players').LocalPlayer.Character, targetHRP.Parent}
...
local __fioletus_aimGyro = nil
local function ensureAimGyro(hrp)
	if not hrp then return end
	if __fioletus_aimGyro and __fioletus_aimGyro.Parent then return __fioletus_aimGyro end
	pcall(function() if __fioletus_aimGyro then __fioletus_aimGyro:Destroy() end end)
	local bg = Instance.new('BodyGyro')
	bg.Name = 'Fioletus_AimGyro'
	bg.P = 20000
	bg.D = 1000
	bg.MaxTorque = Vector3.new(1e7,1e7,1e7)
	bg.Parent = hrp
	__fioletus_aimGyro = bg
	return bg
end
local function destroyAimGyro()
	if __fioletus_aimGyro then pcall(function() __fioletus_aimGyro:Destroy() end) end
	__fioletus_aimGyro = nil
end
