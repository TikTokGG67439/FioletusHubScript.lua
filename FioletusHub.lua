...
-- === Remote logic loader (inserted early so logic functions are available) ===
local remote1 = "https://raw.githubusercontent.com/TikTokGG67439/FioletusHubScript.lua/refs/heads/main/FioletusLogic1.lua"
local remote2 = "https://raw.githubusercontent.com/TikTokGG67439/FioletusHubScript.lua/refs/heads/main/FioletusLogic2.lua"
local function tryLoadFromUrl(url)
    local ok, body = pcall(function()
        if type(game) == 'table' and game.HttpGet then return game:HttpGet(url) end
        if HttpGet then return HttpGet(url) end
        if syn and syn.request then local r = syn.request({Url=url, Method='GET'}); return r and r.Body or nil end
        return nil
    end)
    if ok and body and #body > 10 then
        local fnOk, fn = pcall(function() return loadstring(body) end)
        if fnOk and type(fn) == 'function' then pcall(fn); return true end
    end
    return false
end
pcall(function() tryLoadFromUrl(remote1); tryLoadFromUrl(remote2) end)
-- end loader (early)
...
-- === Remote logic loader ===
-- Attempts to load FioletusLogic1 and FioletusLogic2 from raw GitHub URLs (as you requested).
-- If HttpGet is blocked, this will gracefully fail; files FioletusLogic1_extracted.lua.txt and FioletusLogic2_extracted.lua.txt are provided locally as fallback.
local remote1 = "https://raw.githubusercontent.com/TikTokGG67439/FioletusHubScript.lua/refs/heads/main/FioletusLogic1.lua"
local remote2 = "https://raw.githubusercontent.com/TikTokGG67439/FioletusHubScript.lua/refs/heads/main/FioletusLogic2.lua"

local function tryLoadFromUrl(url)
    local ok, body = pcall(function()
        -- prefer game:HttpGet if available in environment
        if type(game) == 'table' and game.HttpGet then
            return game:HttpGet(url)
        elseif HttpGet then
            return HttpGet(url)
        else
            -- some exploit environments provide syn.request; try a few common providers
...
-- Try remote load, otherwise try local fallback (the logic txt files included alongside)
local loaded1 = tryLoadFromUrl(remote1)
local loaded2 = tryLoadFromUrl(remote2)

-- Local fallback execution (Studio only) - will attempt to read from script's own environment if possible
pcall(function()
    if not loaded1 then
        local f1 = script:FindFirstChild("FioletusLogic1_extracted.lua.txt")
        if f1 and f1:IsA('StringValue') then
            local fn = loadstring(f1.Value)
            if type(fn) == 'function' then pcall(fn) end
        end
    end
    if not loaded2 then
        local f2 = script:FindFirstChild("FioletusLogic2_extracted.lua.txt")
        if f2 and f2:IsA('StringValue') then
            local fn2 = loadstring(f2.Value)
