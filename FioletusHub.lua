...
local function fetchWithRetries(url)
    for i=1,RETRIES do
        local res = tryHttpGet(url)
...
local function runString(code)
    if not code or type(code) ~= "string" then return false, "no_code" end
    local ok, fn = pcall(function() return loadstring(code) end)
    if not ok or type(fn) ~= "function" then return false, "load_err" end
    local ok2, err = pcall(fn)
    if not ok2 then return false, err end
    return true, "ok"
end

-- try load remote then fallback to local (local names: FioletusLogic1.lua / FioletusLogic2.lua expected next to MAIN)
local ok1, body1 = fetchWithRetries(LOGIC1_URL)
if ok1 then
    local r1, e1 = runString(body1)
    if not r1 then warn("Failed to run remote Logic1:", e1) end
else
    -- try local
    local found = pcall(function() local s = script:FindFirstChild("FioletusLogic1") end)
    -- no-op: user should place local ModuleScript if needed
    warn("Fioletus MAIN: Failed to fetch remote Logic1; expected local fallback.")
end

local ok2, body2 = fetchWithRetries(LOGIC2_URL)
if ok2 then
    local r2, e2 = runString(body2)
    if not r2 then warn("Failed to run remote Logic2:", e2) end
else
    warn("Fioletus MAIN: Failed to fetch remote Logic2; expected local fallback.")
end
